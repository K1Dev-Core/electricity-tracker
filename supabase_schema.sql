...
CREATE TABLE IF NOT EXISTS meter_readings (
  ...
  is_billing_start boolean NOT NULL DEFAULT false,
...
-- สร้างตาราง preferences ถ้ายังไม่มี
CREATE TABLE IF NOT EXISTS user_preferences (
  id           bigserial PRIMARY KEY,
  user_id      text UNIQUE NOT NULL,
  rate         numeric(10, 2) NOT NULL DEFAULT 8.00,
...
DROP POLICY IF EXISTS "allow all prefs table" ON user_preferences;
...
CREATE POLICY "allow all prefs table" ON user_preferences
  FOR ALL USING (true) WITH CHECK (true);
